...
std::vector<long long> LLittleFunction(const std::vector<std::string>& pattern, const std::vector<long long int> &n_function) {
    if (pattern.size() != n_function.size()) {
        throw std::logic_error("N function must have same size with pattern");
    }
...
std::vector<long long> NFunction(const std::vector<std::string>& pattern) {
    std::vector<long long> z_function = ZFuncion(pattern.rbegin(), pattern.rend());
...
std::vector<long long> ZFunction (const std::vector<std::string>& pattern) {
    ...
    for (int i = 1, l = 0, r = 0; i < pattern.size(); i++) {
        if (i <= r)
            z_function[i] = (r - i + 1 < z_function[i - l]) ? r - i + 1 : z_function[i - l];
        while (i + z_function[i] < pattern.size() && pattern[z_function[i]] == pattern[i + z_function[i]])
            ++z_function[i];
